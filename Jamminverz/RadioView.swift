    ...
    private var headerView: some View {
        VStack(spacing: 16) {
            HStack {
                ...
                // GlobalControlButtons on the right
...
// MARK: - Global Control Buttons (Temporary - should be moved to separate file)
...
#Preview {
    RadioView(
        taskStore: TaskStore(),
        currentTab: .constant("radio"),
        showSampleListView: .constant(false),
        isGridView: .constant(false),
        showAllSequencersView: .constant(true),
        showColorPicker: .constant(false),
        gridColorMode: .constant("rainbow"),
        selectedGridColor: .constant(.blue)
    )
}